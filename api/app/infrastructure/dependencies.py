...
def require_applicant(user: User = Depends(require_auth_user)) -> User:
    if user.role.title not in [UserRoles.APPLICANT, UserRoles.MODERATOR]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Ошибка доступа')

    return user


def require_employer(user: User = Depends(require_auth_user)) -> User:
    if user.role.title not in [UserRoles.EMPLOYER, UserRoles.MODERATOR]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Ошибка доступа')

    return user


def require_moderator(user: User = Depends(require_auth_user)):
    if user.role.title != UserRoles.MODERATOR:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Ошибка доступа')
