...
from app.domain.entities.users import UserRead, UserActivate, UserUpdate, PasswordChangeRequest
...
class UsersService:
    ...
    async def update(self, user_id: int, user: UserUpdate, current_user: User):
        ...
        if current_user.id != user_model.id and not (
                current_user.role.title == self.user_roles.MODERATOR and current_user.is_admin
        ):
            raise HTTPException(
